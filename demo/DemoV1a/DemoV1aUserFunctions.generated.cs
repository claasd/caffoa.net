    ...
    public class DemoV1aUserFunctions
    {
            ...
            HttpRequest request)
        {
            try {
                ...
                {
                    "simple" => instance.UserPostAsync(_jsonParser.ToObject<L1User>(jToken), request.HttpContext.RequestAborted),
                    "guest" => instance.UserPostAsync(_jsonParser.ToObject<L1GuestUser>(jToken), request.HttpContext.RequestAborted),
            ...
            HttpRequest request, string userId)
        {
            try {
                ...
                var task = discriminator switch
                {
                    "simple" => instance.UserPutAsync(userId, _jsonParser.ToObject<L1User>(jToken), request.HttpContext.RequestAborted),
                    "guest" => instance.UserPutAsync(userId, _jsonParser.ToObject<L1GuestUser>(jToken), request.HttpContext.RequestAborted),
                    _ => throw _errorHandler.WrongContent("type", discriminator, new [] { "simple", "guest" })
                };
