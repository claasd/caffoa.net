...
namespace DemoV1b
{
    ...
    public class DemoV1bUserFunctions
    {
            ...
            HttpRequest request)
        {
            try {
                var instance = _factory.Instance(request);
                var parsedRequestBody = await _jsonParser.Parse<L2GroupedOneOf>(request.Body);
                var caffoaResultParameter = new CaffoaResultHandlerParameter(
                    new int[] { 200 },
                    new string[] { "application/json" },
                    request,
            ...
            HttpRequest request)
        {
            try {
                var instance = _factory.Instance(request);
                var parsedRequestBody = await _jsonParser.Parse<IEnumerable<L2AnyUser>>(request.Body);
                var caffoaResultParameter = new CaffoaResultHandlerParameter(
                    new int[] { 200 },
                    new string[] { "application/json" },
                    request,
